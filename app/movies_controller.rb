...
def can_find_using_where_clause_and_be_sorted
  ...
  Movie.where("release_date > ?",2002).order(release_date: :desc) #Note: in the release_date > ? string, must omit symbol sign ":"
end
...
def can_destroy_a_single_item
  ...
  id = Movie.find_by(title: "That One Where the Guy Kicks Another Guy Once").id #retrieving id of the row that was created
  Movie.destroy(id)
end
